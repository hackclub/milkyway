...
$effect(() => {
  const owned = Object.values(furnitureOwnership).filter(/** @param {any} f */ f => f.owned > 0);
  // Only show purchasable items in the "Buy Furniture" section
  const unowned = Object.values(furnitureOwnership).filter(/** @param {any} f */ f => f.owned === 0 && f.purchasable !== false);
          ...
          {#each ownedFurniture as furniture}
            <div class="furniture-item owned">
              <div class="furniture-image-container">
                <img class="furniture-image" src="/room/{furniture.type}.png" alt={furniture.name} />
              </div>
                ...
                <div class="furniture-actions">
                  {#if furniture.placed < furniture.owned}
                    <button 
                      class="place-button" 
                      onclick={() => {
                        const unplaced = furniture.instances.find(/** @param {any} f */ f => !f.isPlaced);
                  ...
                  {#if furniture.purchasable !== false}
                    <button 
                      ...
                      disabled={userCurrency.paintchips < furniture.cost || purchasingItems.has(furniture.type)}
                      title="Buy another"
                    >
