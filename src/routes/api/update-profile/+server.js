...
const ALLOWED_WALL_VARIANTS = ['default', 'pink', 'clouds', 'mimo', 'glitched', 'gradient'];

export async function POST({ request, cookies }) {
	try {
		const { username, email, bio, githubUsername, howDidYouHear, doingWell, improve, wallVariant } =
			await request.json();
		...
		// Update wall variant if provided
		if (wallVariant !== undefined) {
			if (typeof wallVariant !== 'string' || !ALLOWED_WALL_VARIANTS.includes(wallVariant)) {
				return json(
					{
						success: false,
						error: 'Invalid wall variant'
					},
					{ status: 400 }
				);
			}
			updateData.wallVariant = wallVariant;
		}
