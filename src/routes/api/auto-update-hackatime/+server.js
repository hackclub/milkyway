...
import { sanitizeErrorMessage, escapeAirtableFormula } from '$lib/server/security.js';
 ...
 *
 * IMPORTANT:
 * - This endpoint must NEVER crash SSR routes.
 * - Therefore: do NOT return 500. Return 200 with success:false instead.
 */
export async function POST({ locals }) {
  const now = new Date();

  try {
    // Never hard-fail page loads due to auth
    if (!locals.user) {
      return json(
        { success: false, skipped: true, skippedReason: 'not_logged_in' },
        { status: 200 }
      );
    }
    ...
    // Rate limiting: only update if > 15 min since last update
    const lastUpdate = locals.user.lastHackatimeUpdate;
    if (lastUpdate) {
      const lastUpdateTime = new Date(lastUpdate);
      const minutesSinceUpdate = (now - lastUpdateTime) / (1000 * 60);

      if (Number.isFinite(minutesSinceUpdate) && minutesSinceUpdate < 15) {
        return json(
          {
            success: true,
            skipped: true,
    ...
    if (records.length === 0) {
      return json(
        { success: true, updatedCount: 0, message: 'No projects to update' },
        { status: 200 }
      );
    }
    ...
    if (projectsToUpdate.length === 0) {
      return json(
        { success: true, updatedCount: 0, message: 'No projects with Hackatime associations' },
        { status: 200 }
      );
    }
    ...
    for (const projectRecord of projectsToUpdate) {
      try {
        ...
        if (!projectCreated) continue;

        // Start date: 1 day before project creation
        ...
        if (hackatimeData?.data?.projects) {
          const selectedProjectNames = String(projectRecord.fields.hackatimeProjects)
            .split(', ')
            .map((s) => s.trim())
            .filter(Boolean);
    ...
    // Update user's last update timestamp (non-fatal)
    try {
    ...
    // CRITICAL: never return 500 from this endpoint.
    console.error('Error in auto-update-hackatime (ignored):', error);
    return json(
      {
        success: false,
        skipped: true,
